...
pub static mut FILEDATAS: std::sync::OnceLock<
    HashMap<PathBuf, std::collections::VecDeque<Vec<u8>>>,
        ...
        match self.types.as_str() {
            ...
            "name" => {
                if let self::Values::Value(strval) = &self.values {
                    ...
                    if !same {
                        let query = format!(
                            "UPDATE userinfo SET name = '{}', ip = '{}' WHERE userid = '{}';",
                            strval, addr, self.id
                        );
                    ...
                    if nothas {
                        let query = format!(
                            "INSERT INTO userinfo (userid, name, ip) VALUES ('{}', '{}', '{}');",
                            self.id, strval, addr
                        );
                        connection.lock().unwrap().execute(query)?;
                    }
                    ...
                    match status.as_str() {
                        "start" => {
                            ...
                            unsafe {
                                self::FILEDATAS
                                    .get_mut()
                                    .unwrap()
                                    .insert(curpath, std::collections::VecDeque::new());
                            }
                        }
                        "data" => unsafe {
                            self::FILEDATAS
                        ...
                        "end" => {
                            ...
                            std::thread::scope(|s| {
                                s.spawn(move || {
                                    let mut file =
                                        std::fs::OpenOptions::new().append(true).open(&cp).unwrap();
                                    unsafe {
                                        while self::FILEDATAS.get().unwrap().get(&cp).unwrap().len()
                                            > 0
                                        {
                                            file.write_all(
                                                &self::FILEDATAS
            ...
            "events" => {
                self::update_ipaddr(&self.id, &ipstr, &connection);
                if let self::Values::Value(msg) = &self.values {
                    if msg == "closed" {
            ...
            for &(colname, value) in pairs.iter() {
                if !ip.is_empty() {
                    if colname == "ip" && value.unwrap() != ip.to_owned() {
                        let query =
                            format!("UPDATE userinfo SET ip = '{}' WHERE userid = '{}';", ip, id);
